...
async def signup(signup_request:SignupRequest):
    ...
    print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
    try:
        logger.info("Checking and creating user table if not exists")
        # Check and create the user table if not exists
        check_and_create_user_table()

        logger.info(f"Checking if email already exists: {signup_request.email}")
        # Check if the email already exists
        check_query = f"""SELECT COUNT(*) FROM users WHERE lower(email) = '{signup_request.email.lower()}'"""
        cursor.execute(check_query)
        result = cursor.fetchone()
        logger.info(f"Email existence check result: {result}")

        if result[0] > 0:
            logger.warning(f"User with email '{signup_request.email}' already exists.")
        ...
        logger.info("User inserted successfully, committing transaction")
